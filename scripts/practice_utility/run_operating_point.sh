#!/usr/bin/env bash
# Stage 14: operating point (fixed @512) and curricula @256 under the lossless regime.
set -uo pipefail
...
cell() { # tag iterations modes...
  local tag=$1 it=$2; shift 2
  say "stage 14 [$tag]: $* x 3 seeds x $it it, lossless regime"
  python scripts/practice_utility/run_curriculum_comparison.py \
    ...
    --seeds 8600 8601 8602 --modes "$@" --tag "$tag" --min-free-mib "$MIN_FREE" --execute \
    --extra-overrides $OV > "$OUT/${tag}_stdout.log" 2>&1
  local receipt; receipt=$(grep -o "receipt .*json" "$OUT/${tag}_stdout.log" | tail -1 | cut -d' ' -f2)
  say "stage 14 [$tag] training exit=$? receipt=$receipt"
  [ -z "$receipt" ] && { say "STAGE 14 [$tag] FAILED"; return 1; }
  ...
  say "stage 14 [$tag] eval exit=$?; receipt: $(grep -o 'receipt .*json' "$OUT/${tag}_eval_stdout.log" | tail -1)"
}
cell fixed_512 512 fixed
cell curricula_256 256 lucid ta_lucid_50
say "=== stage 14 done ==="
