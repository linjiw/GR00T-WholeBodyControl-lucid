#!/usr/bin/env bash
# Stage 12: DR arms at 128 it under the combined (lossless) regime.
set -uo pipefail
...
say "stage 12 [regime2_128]: fixed lucid ta_lucid_50 x 3 seeds x 128 it, combined regime"
python scripts/practice_utility/run_curriculum_comparison.py \
  ...
  --seeds 8600 8601 8602 --modes fixed lucid ta_lucid_50 --tag regime2_128 --min-free-mib "$MIN_FREE" --execute \
  --extra-overrides ++algo.config.adaptive_lr_min=1e-6 ++algo.config.adaptive_lr_max=2e-5 ++algo.config.num_learning_epochs=1 \
  > "$OUT/regime2_128_stdout.log" 2>&1
code=$?
receipt=$(grep -o "receipt .*json" "$OUT/regime2_128_stdout.log" | tail -1 | cut -d' ' -f2)
say "stage 12 training exit=$code receipt=$receipt"
[ -z "$receipt" ] && { say "STAGE 12 FAILED"; exit 1; }
python scripts/practice_utility/run_curriculum_robustness_eval.py \
  ...
  --presets id_clean dr_050 dr_full latency_60ms --min-free-mib "$MIN_FREE" --execute > "$OUT/regime2_128_eval_stdout.log" 2>&1
say "stage 12 eval exit=$?; receipt: $(grep -o 'receipt .*json' "$OUT/regime2_128_eval_stdout.log" | tail -1)"
say "=== stage 12 [regime2_128] done ==="
